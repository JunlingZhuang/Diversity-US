...
onMounted(async () => {
  const languageDataPath = new URL(
    "../assets/data/Languages Spoken.csv",
    import.meta.url
  ).href;

  language_data.value = (await d3.csv(languageDataPath)).map((d) => {
    return {
      ...d,
  ...
  console.log("Loaded language data:", language_data.value);
